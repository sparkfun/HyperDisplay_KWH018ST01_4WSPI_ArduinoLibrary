...
void setup() {
  ...
  SERIAL_PORT.println("Example4 HyperDisplay : SparkFun TFT LCD 1.8in Breakout");
  ...
  ILI9163C_color_18_t smallBGcolor = myTFT.rgbTo18b( 0, 127, 200 );       // Make a background color for small window
  ...
  ILI9163C_color_18_t defaultBGcolor = myTFT.rgbTo18b( 220, 0, 64 );     // Make a background color for small window
  ...
  myTFT.line(0,0, 55,45);               // Draw with the same exact function
  delay(1000);                          // Pause for dramatic effect

  myTFT.pCurrentWindow = &smallWindow;  // Switch back to small window
  myTFT.rectangle(0,0, 31, 63);         // Draw a border around the window
  myTFT.pCurrentWindow = &hyperdisplayDefaultWindow;  // Switch back to default window
  myTFT.rectangle(0,0, 127, 159);         // Draw a border around the window
